...
class ReportController extends Controller
{
    ...
    {
        $data = Practitioner::with(['appointments.service'])
            ->get()
            ->map(function ($pr) {
                $totalAppointments = $pr->appointments->count();

                $totalMinutes = $pr->appointments->sum(function ($appt) {
                    // احسب الفرق بين وقت البداية والنهاية
                    $start = Carbon::parse($appt->appointment_date . ' ' . $appt->appointment_time);
                    $end   = Carbon::parse($appt->appointment_date . ' ' . ($appt->end_time ?? $appt->appointment_time));
                ...
                return [
                    ...
                    'average_duration'   => $totalAppointments > 0 ? round($totalMinutes / $totalAppointments, 2) : 0,
                    'revenue'            => $revenue,
                ];
            });
    ...
    public function cancellationNoShow(Request $request)
    {
        ...
        $stats = Practitioner::select('practitioners.id', 'practitioners.name')
            ...
            ->addSelect('cxl.cancelled_count', 'tot.total_count')
            ->get()
            ->map(function ($row) {
        ...
        $statsNoShow = Practitioner::select('practitioners.id', 'practitioners.name')
            ->leftJoinSub($noShows, 'ns', function ($join) {
                $join->on('practitioners.id', '=', 'ns.practitioner_id');
            })
            ->addSelect('ns.no_show_count')
            ->get()
            ->map(function ($row) {
                return [
                    'practitioner_id' => $row->id,
