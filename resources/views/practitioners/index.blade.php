...
@section('content')
    ...
    <ul>
        @foreach ($practitioners as $p)
            <li>
                <strong>Name:</strong> {{ $p->user->name}}<br>
                ...
                <br>
                <a href="{{ route('practitioners.edit', $p->id) }}">Edit</a>
                <form method="POST" action="{{ route('practitioners.destroy', $p->id) }}" style="display:inline">
                    @csrf
                    @method('DELETE')
